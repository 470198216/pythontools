...
def refresh():
    ...
    if mode0 == 0:
        if mode1 == 1:
            l.config(text=get_windows_ip()[0], width=100)
            #l.config(text=get_net_data()[0], width=8)
        elif mode1 == 0:
            l.config(text=get_windows_ip()[1], width=100)
...
def change_mode(e):
    ...
    print("change mode0:", mode0, "1:%d" ,mode1)
...
#         fl.close()
    ...
    root = tk.Tk()
    root.geometry('200x30+{}+{}'.format(pos_x, pos_y))  # 窗口初始位置
